...
def parse_cli_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="LoRA SFT for DeepSeek-OCR-2.")
    parser.add_argument(
        "--config",
        type=str,
        default=str(workspace / "lora_sft_config.yaml"),
        help="Path to lora sft YAML config.",
    )
...
def _maybe_launch_with_accelerate(cfg: LoRASFTConfig, cli_args: argparse.Namespace) -> None:
...
def main() -> None:
    ...
    if trainer.is_world_process_zero():
        ...
        print(f"Saved LoRA fine-tuned weights to: {cfg.output_dir}")
